...
### Changed
- **Pricing page now states the trial and tax terms.** Each hosted plan spells out the **60-day free trial** and the **full recurring price that applies after it** ($7/month or $60/year per member), and the page notes that **prices exclude tax — applicable taxes are calculated at checkout**. This brings the public pricing in line with what a Merchant-of-Record checkout (Paddle) shows the buyer, which their domain review requires.
...
### Fixed
- **Download CSV is fully gated when nothing is selected.** Disabling the button alone wasn't enough — the button is wrapped in an `Anchor`, and clicking a disabled button inside an `<a>` still triggers the link's navigation. Now the `Anchor`'s `href` (the `StreamResource`) is also detached when the selection is empty and re-attached only when the user picks at least one date bound or checks *All my data*. Admin tab additionally requires a member to be selected. Open-ended ranges (from-only / to-only) are fully supported by the service — from-only exports forward to the most recent post (including in-progress); to-only exports from the beginning of time up to that date.
...
- **Encryption-trade-off warning in Admin.** The Slack publishing section in Admin → Integrations now shows a prominent warning that enabling publishing means content leaves the encrypted-at-rest database and becomes visible in the Slack workspace.
- **Per-member Slack publishing preferences.** When the group master toggle is on, members see a *Slack publishing* section in their profile with two checkboxes (publish my stats, publish my post body). If the group also requires one of those at the global level, the matching member checkbox is force-checked and read-only — and a visible *"Required by group settings"* badge is shown next to it so the override is obvious without hovering. The member's stored value is preserved so flipping the override off later restores their original preference.
- **Body-before-stats ordering with consistent multi-line indentation.** When both body and stats are sent, the post body appears first, followed by the stats block. Each body field renders with its label on its own line and its value as a Slack blockquote underneath; every newline-separated line of the value gets its own `>` prefix so long, multi-paragraph entries (especially *Worst*) stay flush under their section header instead of breaking out of the quote block after the first line.
...
### Changed
- **Consistent Save button styling.** The profile Save button and the Admin Integrations Save button now both read *"Save Changes"* with the primary theme variant and a check icon, so the two settings screens feel uniform.
...
- **Fix: all credentials now embedded in `environment:` block.** The CLI template (`docker-compose.yml.tmpl`) no longer uses `env_file`. Per-group Keycloak registration properties are now set as direct Spring property overrides (`SPRING_SECURITY_OAUTH2_CLIENT_REGISTRATION_KEYCLOAK_*`) alongside the provider URI overrides that were already there. All other per-group values (DB credentials, Flyway settings, Vaadin mode, notify schedule) are also in the `environment:` block. The encryption master key uses Docker Compose variable interpolation (`${TNRA_ENCRYPTION_MASTER_KEY}`), read from `~/tnra/.env` when the command runs from that directory.
- **`env.tmpl` restored to local-dev-only purpose.** The generated `.env` file in `provision/<group-name>/` is for IDE development only, not loaded by the production docker-compose. Uses `localhost:3307` for the host-mapped MySQL port.
- **`build.context` corrected.** Changed from `../..` to `.` — Docker Compose resolves `build.context` relative to the project directory (first `-f` file), so `.` correctly references the project root where the `Dockerfile` lives.
- **PRODUCTION guides cleaned up.** Files stay in `provision/<group-name>/` locally and on the VPS (copied to `~/tnra/provision/`). All commands run from `~/tnra/` using `docker compose -f docker-compose.production.yml -f provision/<group-name>/docker-compose.yml`. Group container commands use `docker logs`/`docker restart` with the container name `tnra-<group-name>`. Client secret rotation instructions updated to edit the embedded `SPRING_SECURITY_OAUTH2_CLIENT_REGISTRATION_KEYCLOAK_CLIENT_SECRET` in the group's `docker-compose.yml`.
